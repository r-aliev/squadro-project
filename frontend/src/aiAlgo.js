...
function makeMove(board, i) {
	...
	if (boardClone.turn == true) {
		...
		if (boardClone.computer.pieces[i].sens == 1) {
			...
			for (j = boardClone.computer.pieces[i].y + 1; j <= 6; j++) {
				if (j === 6) {
					...
					precedentVide = false;
				} else if (precedentVide === false && boardClone.plateau.pieces[i + 1][j] == null) {
			...
			for (j = boardClone.computer.pieces[i].y - 1; j >= 0; j--) {
				if (j === 0) {
					...
					deplace(boardClone, boardClone.computer.pieces[i], j);
					return boardClone;
				} else if (precedentVide === true && j === boardClone.computer.pieces[i].y - boardClone.computer.pieces[i].move) {
					deplace(boardClone, boardClone.computer.pieces[i], j);
					return boardClone;
				}
			}
		...
		//partie joueur
		if (boardClone.player.pieces[i].sens === -1 && boardClone.player.pieces[i].x === 6) return null; //c'est le cas ou le pion a deja fait un aller/retour
		if (boardClone.player.pieces[i].sens == 1) {
			for (j = boardClone.player.pieces[i].x - 1; j >= 0; j--) {
				if (j === 0) {
					...
					precedentVide = false;
				} else if (precedentVide === false && boardClone.plateau.pieces[j][i + 1] == null) {
					deplace(boardClone, boardClone.player.pieces[i], j);
					return boardClone;
				} else if (precedentVide === true && j === boardClone.player.pieces[i].x - boardClone.player.pieces[i].move) {
					deplace(boardClone, boardClone.player.pieces[i], j);
					return boardClone;
				}
			}
